...
(() => {
  ...
  const getPoints=user=>{const p=user.user_metadata||{},stored=Number(p.points||0),last=Date.parse(p.points_last_at||'');return Number.isFinite(last)?stored+Math.max(0,Math.floor((Date.now()-last)/60000)):stored};
  async function updatePoints(user){if(!user)return user;const profile=user.user_metadata||{},hasLast=Number.isFinite(Date.parse(profile.points_last_at||'')),points=getPoints(user),last=Date.parse(profile.points_last_at||'');if(!hasLast){const {data}=await db.auth.updateUser({data:{points,points_last_at:new Date().toISOString()}});return data.user||user}if(points===Number(profile.points||0)&&Date.now()-last<60000)return user;const {data}=await db.auth.updateUser({data:{points,points_last_at:new Date().toISOString()}});return data.user||user}
