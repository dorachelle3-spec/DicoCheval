...
(() => {
  ...
  const activeNow=()=>document.visibilityState==='visible'&&document.hasFocus()&&Date.now()-lastUserActivity<30000;
  ...
  async function updatePoints(user){if(!user||pointsBusy)return user;pointsBusy=true;try{const profile=user.user_metadata||{},now=Date.now(),checkpoint=Date.parse(profile.active_points_checkpoint||'')||now,reset=user.id===OWNER&&!profile.activity_points_v2;let stored=reset?0:getPoints(user),millis=reset?0:Math.max(0,Number(profile.active_points_millis||0));const elapsed=activeNow()?Math.min(Math.max(0,now-checkpoint),30000):0,total=millis+elapsed,earned=Math.floor(total/60000),data={points:stored+earned,active_points_millis:total%60000,active_points_checkpoint:new Date(now).toISOString(),activity_points_v2:true};if(reset||earned||!profile.activity_points_v2||!Number.isFinite(Date.parse(profile.active_points_checkpoint||''))){const result=await db.auth.updateUser({data});return result.data.user||user}return user}finally{pointsBusy=false}}
  ...
  setInterval(async()=>{const {data}=await db.auth.getUser();if(data.user){currentUser=await updatePoints(data.user);if(modal.classList.contains('open'))renderAvatars();refreshButton()}},30000);
