(() => {
  ...
  async function loadEvents(){
    ...
    const today=new Date().toISOString().slice(0,10), upcoming=(data||[]).filter(event=>event.event_date>=today);
  ...
  async function deleteEvent(id){
    ...
    if(error)return status('La suppression n’a pas fonctionné. Réessaie.'); status('Événement supprimé.',true);loadEvents();
  }
  async function saveEvent(){
    ...
    const {error}=await db.from('dog_calendar_events').insert({user_id:user.id,event_date:eventDate,event_time:$('calTime').value||null,title,event_type:$('calType').value,notes:$('calNotes').value.trim()||null});
    ...
    $('calTitle').value='';$('calTime').value='';$('calNotes').value='';status('Événement ajouté à ton calendrier privé.',true);loadEvents();
  }
