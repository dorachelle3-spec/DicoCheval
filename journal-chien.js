(() => {
  ...
  $('new').onclick=blank;
  $('deleteEntry').onclick=async()=>{if(!currentEntry||!confirm('Supprimer définitivement cette page ?'))return;const {error}=await db.from('dog_journal_entries').delete().eq('id',currentEntry.id);if(error){status('Impossible de supprimer cette page.');return}$('entryBook').classList.remove('open');currentEntry=null;status('Page supprimée.');await load()};
  $('backLibrary').onclick=()=>$('entryBook').classList.remove('open');
